...
import { useRoqClient, useFeedbackFindFirst } from 'lib/roq';
...
function FeedbackEditPage() {
  ...
  const { data, error, isLoading, mutate } = useFeedbackFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: FeedbackInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.feedback.update({
        data: values as RoqTypes.feedback,
        where: {
          id,
        },
      });
