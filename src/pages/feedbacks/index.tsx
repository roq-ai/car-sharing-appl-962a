...
import { useRoqClient, useFeedbackFindManyWithCount } from 'lib/roq';
...
export function FeedbackListPage(props: FeedbackListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useFeedbackFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.feedback.delete({ where: { id } });
      await mutate();
    } catch (error) {
