...
import { useRoqClient, useServiceRequestFindFirst } from 'lib/roq';
...
function ServiceRequestEditPage() {
  ...
  const { data, error, isLoading, mutate } = useServiceRequestFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ServiceRequestInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.service_request.update({
        data: values as RoqTypes.service_request,
        where: {
          id,
        },
      });
