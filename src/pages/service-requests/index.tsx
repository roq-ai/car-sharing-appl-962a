...
import { useRoqClient, useServiceRequestFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ServiceRequestInterface, unknown>;
...
export function ServiceRequestListPage(props: ServiceRequestListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['request_type.contains', 'description.contains', 'status.contains'],
          ...params.filters,
        },
        'service_request',
      ),
  ...
  const { data, error, isLoading, mutate } = useServiceRequestFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.service_request.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ServiceRequestInterface) => {
    if (hasAccess('service_request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/service-requests/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'request_type', header: 'Request Type', accessorKey: 'request_type' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/service-requests/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('service_request', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/service-requests/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('service_request', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="service_request" />
        </Flex>

        {hasAccess('service_request', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/service-requests/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
